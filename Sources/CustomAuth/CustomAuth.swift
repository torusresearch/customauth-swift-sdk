...
open class CustomAuth {
    ...
    func newGetAggregateTorusKey(verifier: String, verfierId: String, subVerifierInfoArray: [TorusSubVerifierInfo], userData: [String: Any] = [:]) async throws -> [String: Any] {
        ...
        let buffer: Data = try! NSKeyedArchiver.archivedData(withRootObject: extraParams, requiringSecureCoding: false)
        do {
            let nodeDetails = try await getNodeDetailsFromContract(verifier: verifier, verfierID: verfierId)
            let responseFromRetrieveShares = try await torusUtils.retrieveShares(torusNodePubs: nodeDetails.getTorusNodePub(), endpoints: nodeDetails.getTorusNodeEndpoints(), verifier: verifier, verifierId: verfierId, idToken: aggregateIdToken, extraParams: buffer)
            var data = userData
            data["privateKey"] = responseFromRetrieveShares["privateKey"]
            data["publicAddress"] = responseFromRetrieveShares["publicAddress"]
            return data
...
public struct AggregateVerifierParams {
    var veriferParams: [VerifierParams]
